...
## 06.redis 관련 code 작성 및 배포 2
<details>
<summary>펼치기/접기</summary>

Redis에 값을 추가하는 기능을 구현해본다.  
프로덕트가 생기면 최저가를 구할 때 그 프로덕트의 가격도 포함되어서 검색이 되야한다.  
저장할 sorted set에도 가격까지 같이 sort가 되어서 결과값이 나와야 하기 때문에 가격이 꼭 필요하다.

### Redis 상품 데이터 추가 기능 API

#### 상품 목록 추가 API 메소드 컨트롤러 로직 
PUT 방식으로 구현한다.
PUT 방식은 추가하려는 데이터가 없으면 새로 추가하고, 있으면 새롭게 갱신하는것과 같이 "멱등 해야한다" 는 의미적 정의인 멱등성을 가진다.    
현재 구현하려는 ZSET의 ZADD의 경우 ZSET의 중복을 허용하지 않는 특성으로 동일한 MEMBER가 이미 있다면, 해당 MEMBER에 새로운 SCORE를 덮어씌운다.    
이는 자바의 Map 자료구조의 put의 의미와 같다. (이미 key가 있다면 해당 key에 새로운 value를 덮어씌운다.)
  ...
  ```
#### 상품 목록 추가 기능 메소드 Service 로직
  1. redisTemplate로 부터 ZSetOperations 객체를 반환받는다.  
  2. 반환받은 ZSETOperations 객체에서 add 메소드를 호출하여 member, key, score 순으로 저장할 값을 매개변수로 차례로 기입한다.
  3. 2번을 통해 저장 완료 후 ZSetOperations 객체로부터 rank메소드를 호출하여 member, key 순으로 매개변수를 차례로 기입한뒤 해당 값을 반환한다.  
  ```java
  @Service
  @RequiredArgsConstructor
  public class LowestPriceServiceImpl implements LowestPriceService {
...
#### Swagger를 통한 값 추가 API 호출  
  - URL: [swagger-ui.html](http://localhost:8080/swagger-ui.html#/lowest-price-controller)
  - [POST] /product 에서 [Try it Out]  튼 클릭
    ![img.png](swagger-PUT-product.png)
  - Example Value 영역에 저장할 request parameter를 json형태로 입력한 후 Execute 버튼을 클릭한다. 
    ![img.png](swagger-PUT-product-execute.png)
#### Swagger를 통한 추가된 값 조회 API 호출
  - [GET] /getSETValue 에서 [Try it Out] 버튼 클릭 후 Parameters의 key 영역에 조회할 key값을 입력한 후 Execute 버튼을 클릭한다.
    ![img_1.png](swagger-GET-product-execute.png)  

#### Redis에서 조회
  ```bash
  zrange FPG0001 0 9 withscores
  ``` 
  ![img.png](redis-GET-savedproduct.png)
  저장한 FPG0001 key의 testNewProduct member의 10000 score값이 0~9 범위의 10개 데이터 중 8번째 순위로 조회되는것을 확인할 수 있다.

### Redis 상품 그룹 추가 기능
#### 상품 그룹 추가 API 메소드 컨트롤러 로직
새로운 상품 그룹이 추가된다고 가정했을 때 상품 그룹 기준 상품 목록인 productList에 들어가는 각각의 상품들은 판매자 별 상품 목록이다.  
즉, 같은 상품(Product) id라고 할 지라도 A 판매자(Seller)가 판매하는 상품 id와과 B 판매자(Seller)가 판매하는 상품 id가 다르다.  
하나의 상품 목록에 같은 상품일 지라도 상품 id가 Seller별로 서로 다르게 구성이 된다.  
상품 그룹에 새로운 상품으로 하기스 3단계 기저귀 라는 상품을 판매하기로 가정했을 때, 지금까지의 상품 목록에는 Seller가 하나도 없었을것이다.  
그러나 판매자는 해당 상품 그룹에 해당하는 상품을 판매할 것이기 때문에 실제로 상품 그룹이 추가됨과 동시에 판매자가 판매하려는 상품이 함께 추가된다.  
다시말해, 상품 그룹이 추가되는 배경은 한명의 판매자가 신상품을 판매하려고 할때 발생한다.  
따라서 ProductGroup에 대한 정보를 받을 때 상품 목록도 함께 받아야한다.  
  ...
  public class LowestPriceController {
    /**
     * Redis 상품 그룹 데이터 추가 및 추가된 상품 그룹에 해당하는 상품목록 갯수 반환
     * <pre>
     *      ProductGroupId, ProductId, Price 정보를 추가한다.
     *      ProductGroupId
     * </pre>
...
### Redis 키워드 추가 기능
keyword(key) 기준 productGroupId(member)와 score(점수)를 저장한다.

#### 엘라스틱 서치와 score
score는 엘라스틱 서치에서 검색된 검색 결과의 관련성을 평가하는 점수로 사용된다.
엘라스틱 서치는 RDB에서 행 단위와 같은 문서(Document)단위로 데이터를 저장한다.
score(점수)는 TF(키워드 빈도), IDF(단어 회귀도), 문서 길이 등을 고려하여 계산된다.
score(점수)는 저장되지 않고 검색할 때마다 실시간으로 계산된다.
키워드를 검색하는 쿼리를 실행하게 되면, 검색하려는 키워드가 포함되어있는 문서들이 조회된다.
문서에 해당 키워드에 대한 관련성에 대한 점수가 계산되어 문서와 score값을 함께 반환한다.

#### 키워드 추가 API 메소드 컨트롤러 로직
  ```java
  @PutMapping("/keyword")
...
### Redis 키워드 기준 상품 그룹 별 상품 목록 조회
keyword(key) 기준 productGroupId(member) 목록(10개) 와
productGroupId(member) 목록(10개) 기준 product 목록(10개)를 조회한다.
